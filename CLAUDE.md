...
장르 간 VOC 비교가 가능한 Looker Studio 대시보드로 시각화한다.
...
| AI 분석 | Google Gemini API (gemini-1.5-flash, 무료 티어) |
| 저장 | Google Sheets (gspread + 서비스 계정) |
| 자동화 | GitHub Actions (매일 1회 또는 주 1회) |
| 시각화 | Looker Studio (Google Sheets 연동) |
...
| 브라운더스트2 | 전략 RPG | com.neowiz.browndust2 |
...
├── storage/
│   ├── __init__.py
│   └── sheets.py              # Google Sheets 저장/업데이트
│
├── config.py                  # 게임 설정 (앱 ID, 시트 이름 등)
...
| 시트명 | 내용 |
|--------|------|
| lineage_m | 리니지M 리뷰 분석 결과 |
| browndust2 | 브라운더스트2 리뷰 분석 결과 |
| summary | 두 게임 통합 비교 (Looker Studio 메인 소스) |

---

## 데이터 스펙 (Google Sheets 컬럼)
...
4. **민감 정보 보호** - API 키, 서비스 계정 JSON은 .env 및 GitHub Secrets에만 저장.
...
- Gemini 무료 티어: 분당 15회 요청 제한. 리뷰 배치 처리 시 rate limit 고려.
- google-play-scraper는 비공식 API. Google Play 정책 변경 시 동작 불안정 가능.
- Google Sheets API: 분당 60회 쓰기 제한. 대량 업데이트 시 배치 append 사용.
- 서비스 계정 JSON은 git에 절대 커밋하지 않는다. GitHub Actions에서는 Secrets 활용.
- summary 시트는 lineage_m, browndust2 시트 데이터를 통합한 뷰로 유지한다.
